...
def inv_index(request):
	...
	return render(request, 'matrixolver-bootstrap/inverse.html', {'A': A,})
...
def inv_get_size(request):
	if request.method == 'POST':
		...
		return render(request, 'matrixolver-bootstrap/inverse.html', {'A': A, 'post': post,})
...
def inv_solution(request):
	if request.method == 'POST':
		...
		if 'error' in result:
			...
			return render(request, 'matrixolver-bootstrap/inverse.html', {'sol': result['sol'], 'A': A, 'rows': rows, 'message': error_message['cz'],})
		else:
			return render(request, 'matrixolver-bootstrap/inverse.html', {'sol': result['sol'], 'A': A, 'rows': rows,})
	return inv_index(request)
